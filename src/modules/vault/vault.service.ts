...
export class VaultService {
  ...
  async getVaultHistoryByVaultId(
    ...
    const vaultHistoryRecords = await this.prisma.vaultHistory.findMany({
      where: {
        vaultId: vaultId,
      },
    });
    const vaultKey = await this.prisma.vault.findUnique({
      where: {
        id: vaultId,
      },
      select: {
  ...
  async getVaultHistory(
    ...
    const userVaultStrategies = await this.prisma.vaultStartegy.findMany({
      where: {
        vault_id: vaultId,
        strategy: {
          user_id: userId,
  ...
  async getVaultUserInfo(
    ...
    const userStrategies = await this.prisma.strategy.findMany({
      where: { user_id: userId },
      select: { id: true, name: true },
    });

    const vaultStrategies = await this.prisma.vaultStartegy.findMany({
      where: {
        vault_id: vaultId,
        strategy_id: {
  ...
  private async getTotalStrategyDepositedAmount(strategyIds: string[]) {
    ...
    const result = await this.prisma.vaultStartegy.groupBy({
      by: ['strategy_id'],
      where: {
