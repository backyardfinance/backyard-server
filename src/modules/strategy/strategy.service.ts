...
export class StrategyService {
  ...
  async deleteStrategy(strategyId: string): Promise<void> {
    await this.prisma.$transaction([
      this.prisma.vaultStartegy.deleteMany({
      ...
      for (const [vaultId, depositedAmount] of Object.entries(vaultDeposits)) {
        ...
        const ownershipFraction =
          Number(vault.current_tvl) > 0
            ...
            : 0;
        await this.prisma.vaultStartegy.create({
          data: {
            vault_id: vaultId,
  ...
  async getStrategiesVaults(strategyId: string) {
    return this.prisma.vaultStartegy.findMany({
      where: {
        strategy_id: strategyId,
      },
      include: {
  ...
  public async getStrategyHistory(
    ...
    const strategyVaults = await this.prisma.vaultStartegy.findMany({
      where: { strategy_id: strategyId },
      select: {
        vault_id: true,
    ...
    for (const vaultId of vaultIds) {
      vaultHistoriesByVault[vaultId] = await this.prisma.vaultHistory.findMany({
        where: {
          vaultId,
          recorded_at: {
